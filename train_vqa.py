...
def main(opts):
    ...
    #                                   enabled=opts.fp16, opt_level='O2') # 原本是O2
    model, optimizer = amp.initialize(model, optimizer,
    ...
    while True:
        for step, batch in enumerate(train_dataloader):
            ...
            vqa_loss, itm_loss, topic_loss, neg_loss, _ = model(batch, compute_loss=True)
            vqa_loss = vqa_loss.mean() * batch['targets'].size(1)  # instance-level bce
            itm_loss = itm_loss * 2
            topic_loss = topic_loss.mean() * 25
            loss = vqa_loss + itm_loss + topic_loss - neg_loss
            ...
            if (step + 1) % opts.gradient_accumulation_steps == 0:
                ...
                if global_step % 100 == 0:
                    ...
                    LOGGER.info(f"Vqa_loss:{vqa_loss}")
                    LOGGER.info(f"Topic_loss:{topic_loss}")
                    LOGGER.info(f"Neg_loss:{neg_loss}")
                    LOGGER.info(f"ITM_loss:{itm_loss}")
...
def validate(model, val_loader, label2ans):
    ...
    for i, batch in enumerate(val_loader):
        scores,_ = model(batch, compute_loss=False)
